...
public final class ColumnInfoRecordsAggregate extends RecordAggregate {
    ...
    private int findStartOfColumnOutlineGroup(int pIdx) {
        ...
        ColumnInfoRecord columnInfo2 = columnInfo;
        int idx = pIdx;
        while (idx != 0) {
            ColumnInfoRecord prevColumnInfo = (ColumnInfoRecord) this.records.get(idx - 1);
            if (!prevColumnInfo.isAdjacentBefore(columnInfo2) || prevColumnInfo.getOutlineLevel() < level) {
                break;
            }
            idx--;
            columnInfo2 = prevColumnInfo;
        }
        return idx;
    }
    ...
    private int findEndOfColumnOutlineGroup(int colInfoIndex) {
        ...
        while (idx < this.records.size() - 1) {
            ...
            if (!columnInfo2.isAdjacentBefore(nextColumnInfo) || nextColumnInfo.getOutlineLevel() < level) {
                break;
            }
            idx++;
            columnInfo2 = nextColumnInfo;
        }
        return idx;
    }
    ...
    public void setColumn(int targetColumnIx, Short xfIndex, Integer width, Integer level, Boolean hidden, Boolean collapsed) {
        ...
        if (z) {
            ...
            if (ci.getFirstColumn() == i || ci.getLastColumn() == i) {
                ...
                ciStart.setLastColumn(i - 1);
                ciMid.setFirstColumn(i);
                ciMid.setLastColumn(i);
                int lastcolumn2 = lastcolumn;
                ColumnInfoRecord ciEnd2 = ciEnd;
                ColumnInfoRecord ciMid2 = ciMid;
                setColumnInfoFields(ciMid, xfIndex, width, level, hidden, collapsed);
                int k2 = k + 1;
                insertColumn(k2, ciMid2);
                ColumnInfoRecord ciEnd3 = ciEnd2;
                ciEnd3.setFirstColumn(i + 1);
                ciEnd3.setLastColumn(lastcolumn2);
                insertColumn(k2 + 1, ciEnd3);
            }
        }
    }
    ...
    private int findColInfoIdx(int columnIx, int fromColInfoIdx) {
        if (columnIx < 0) {
            ...
            sb.append("column parameter out of range: ");
            sb.append(columnIx);
            throw new IllegalArgumentException(sb.toString());
        } else if (fromColInfoIdx < 0) {
            StringBuilder sb2 = new StringBuilder();
            sb2.append("fromIdx parameter out of range: ");
            sb2.append(fromColInfoIdx);
    ...
    public void groupColumnRange(int fromColumnIx, int toColumnIx, boolean indent) {
        ...
        while (i <= toColumnIx) {
            ...
            int colInfoSearchStartIdx2 = colInfoSearchStartIdx;
            setColumn(i, null, null, Integer.valueOf(level2), null, null);
            i++;
            colInfoSearchStartIdx = colInfoSearchStartIdx2;
        }
    }
