...
public class PearsonsCorrelation {
    ...
    public RealMatrix getCorrelationStandardErrors() {
        ...
        double[][] out = (double[][]) Array.newInstance(double.class, new int[]{nVars, nVars});
        ...
        for (int i = 0; i < nVars; i++) {
            for (int j = 0; j < nVars; j++) {
                if (i == j) {
                    ...
                    double r = this.correlationMatrix.getEntry(i, j);
                    out[i][j] = tDistribution.cumulativeProbability(-FastMath.abs(FastMath.sqrt(((double) (this.nObs - 2)) / (1.0d - (r * r))) * r)) * 2.0d;
                }
            }
        }
        return new BlockRealMatrix(out);
    ...
    public RealMatrix computeCorrelationMatrix(double[][] data) {
        return computeCorrelationMatrix((RealMatrix) new BlockRealMatrix(data));
    }
