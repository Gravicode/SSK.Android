...
public abstract class HeaderFooter implements org.apache.poi.p009ss.usermodel.HeaderFooter {
    ...
    public static String stripFields(String pText) {
        ...
        for (MarkupTag mt : MarkupTag.values()) {
            ...
            while (true) {
                int indexOf = text2.indexOf(seq);
                int pos = indexOf;
                if (indexOf <= -1) {
                    break;
                }
                StringBuilder sb = new StringBuilder();
                sb.append(text2.substring(0, pos));
                sb.append(text2.substring(seq.length() + pos));
                text2 = sb.toString();
            }
        }
        return text2.replaceAll("\\&\\d+", "").replaceAll("\\&\".*?,.*?\"", "");
    }
}
