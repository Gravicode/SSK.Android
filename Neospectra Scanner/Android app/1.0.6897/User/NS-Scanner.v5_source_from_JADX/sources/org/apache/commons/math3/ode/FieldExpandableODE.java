...
public class FieldExpandableODE<T extends RealFieldElement<T>> {
    ...
    public void init(T t0, T[] y0, T finalTime) {
        ...
        while (true) {
            ...
            if (index < this.mapper.getNumberOfEquations()) {
                ((FieldSecondaryEquations) this.components.get(index - 1)).init(t0, primary0, this.mapper.extractEquationData(index, y0), finalTime);
            } else {
                return;
            }
        }
    ...
    public T[] computeDerivatives(T t, T[] y) throws MaxCountExceededException, DimensionMismatchException {
        T[] yDot = (RealFieldElement[]) MathArrays.buildArray(t.getField(), this.mapper.getTotalDimension());
        int index = 0;
        ...
        while (true) {
            ...
            this.mapper.insertEquationData(index, ((FieldSecondaryEquations) this.components.get(index - 1)).computeDerivatives(t, primaryState, primaryStateDot, this.mapper.extractEquationData(index, y)), yDot);
        }
    }
}
