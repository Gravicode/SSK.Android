...
public abstract class AbstractShape {
    ...
    public static AbstractShape createShape(HSSFShape hssfShape, int shapeId) {
        ...
        if (hssfShape instanceof HSSFComment) {
            ...
            if (shapeType == 8) {
                shape2 = new PictureShape(simpleShape, shapeId);
            } else if (shapeType != 20) {
                switch (shapeType) {
                    case 1:
                        shape2 = new LineShape(simpleShape, shapeId);
                        break;
                    case 2:
                    case 3:
                        shape2 = new SimpleFilledShape(simpleShape, shapeId);
                        break;
                    default:
                        throw new IllegalArgumentException("Do not know how to handle this type of shape");
                }
            } else {
                shape2 = new ComboboxShape(simpleShape, shapeId);
            }
            shape = shape2;
        } else {
