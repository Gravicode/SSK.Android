...
public class MatrixUtils {
    ...
    private static class BigFractionMatrixConverter extends DefaultFieldMatrixPreservingVisitor<BigFraction> {
        ...
        public void start(int rows, int columns, int startRow, int endRow, int startColumn, int endColumn) {
            this.data = (double[][]) Array.newInstance(double.class, new int[]{rows, columns});
        }
        ...
        if (rows == matrix.getColumnDimension()) {
            for (int i = 0; i < rows; i++) {
                ...
                while (j < rows) {
                    ...
                    int j2 = j;
                    if (FastMath.abs(mij - mji) <= FastMath.max(FastMath.abs(mij), FastMath.abs(mji)) * d) {
                        j = j2 + 1;
                    } else if (raiseException) {
                        throw new NonSymmetricMatrixException(i, j2, d);
                    } else {
                        return false;
                    }
                }
    ...
    public static Array2DRowRealMatrix fractionMatrixToRealMatrix(FieldMatrix<Fraction> m) {
        ...
        m.walkInOptimizedOrder((FieldMatrixPreservingVisitor<T>) converter);
    ...
    public static void deserializeRealMatrix(Object instance, String fieldName, ObjectInputStream ois) throws ClassNotFoundException, IOException {
        try {
            int n = ois.readInt();
            int m = ois.readInt();
            double[][] data = (double[][]) Array.newInstance(double.class, new int[]{n, m});
            for (int i = 0; i < n; i++) {
                double[] dataI = data[i];
                for (int j = 0; j < m; j++) {
    ...
    public static RealMatrix blockInverse(RealMatrix m, int splitIndex) {
        ...
        int splitIndex1 = i + 1;
        RealMatrix a = realMatrix.getSubMatrix(0, i, 0, i);
        RealMatrix b = realMatrix.getSubMatrix(0, i, splitIndex1, n - 1);
        RealMatrix c = realMatrix.getSubMatrix(splitIndex1, n - 1, 0, i);
        RealMatrix d = realMatrix.getSubMatrix(splitIndex1, n - 1, splitIndex1, n - 1);
