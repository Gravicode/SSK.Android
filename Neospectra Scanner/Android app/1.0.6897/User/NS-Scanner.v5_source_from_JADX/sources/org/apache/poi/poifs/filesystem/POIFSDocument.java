...
public final class POIFSDocument implements BATManaged, BlockWritable, POIFSViewable {
    ...
    public POIFSDocument(String name, int size, POIFSBigBlockSize bigBlockSize, POIFSDocumentPath path, POIFSWriterListener writer) {
        ...
        this._property.setDocument(this);
        if (this._property.shouldUseSmallBlocks()) {
            SmallBlockStore smallBlockStore = new SmallBlockStore(this._bigBigBlockSize, path, name, size, writer);
            this._small_store = smallBlockStore;
            this._big_store = new BigBlockStore(this._bigBigBlockSize, EMPTY_BIG_BLOCK_ARRAY);
            return;
        }
        this._small_store = new SmallBlockStore(this._bigBigBlockSize, EMPTY_SMALL_BLOCK_ARRAY);
        BigBlockStore bigBlockStore = new BigBlockStore(this._bigBigBlockSize, path, name, size, writer);
        this._big_store = bigBlockStore;
    }
    ...
    public void read(byte[] buffer, int offset) {
        ...
        int remaining = len;
        int writePos = 0;
        DataInputBlock currentBlock2 = currentBlock;
        ...
        while (true) {
            ...
            currentBlock2.readFully(buffer, writePos, reqSize);
            remaining -= reqSize;
            writePos += reqSize;
            currentOffset += reqSize;
            if (blockIsExpiring) {
                if (currentOffset != this._size) {
                    currentBlock2 = getDataInputBlock(currentOffset);
                    blockAvailable = currentBlock2.available();
                } else if (remaining > 0) {
