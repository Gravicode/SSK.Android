...
class TricubicSplineFunction implements TrivariateFunction {
    ...
    /* renamed from: N */
    private static final short f524N = 4;

    /* renamed from: a */
    private final double[][][] f525a = ((double[][][]) Array.newInstance(double.class, new int[]{4, 4, 4}));

    TricubicSplineFunction(double[] aV) {
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                for (int k = 0; k < 4; k++) {
                    this.f525a[i][j][k] = aV[(((k * 4) + j) * 4) + i];
                }
            }
        }
    }
        ...
        if (x < 0.0d || x > 1.0d) {
            ...
            while (i2 < i) {
                ...
                while (j < i) {
                    int k = 0;
                    while (k < i) {
                        result2 += this.f525a[i2][j][k] * pX[i2] * pY[j] * pZ[k];
                        k++;
                        i = 4;
                    }
                    j++;
