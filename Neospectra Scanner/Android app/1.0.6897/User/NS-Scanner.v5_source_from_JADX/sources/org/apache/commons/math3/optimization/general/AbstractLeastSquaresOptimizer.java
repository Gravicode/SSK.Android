...
public abstract class AbstractLeastSquaresOptimizer extends BaseAbstractMultivariateVectorOptimizer<DifferentiableMultivariateVectorFunction> implements DifferentiableMultivariateVectorOptimizer {
    ...
    /* renamed from: jF */
    private MultivariateDifferentiableVectorFunction f732jF;
    ...
    public RealMatrix computeWeightedJacobian(double[] params) {
        ...
        while (true) {
            ...
            DerivativeStructure derivativeStructure = new DerivativeStructure(nC, 1, i2, params[i2]);
            dsPoint[i2] = derivativeStructure;
            i = i2 + 1;
        }
        DerivativeStructure[] dsValue = this.f732jF.value(dsPoint);
        int nR = getTarget().length;
        if (dsValue.length != nR) {
            throw new DimensionMismatchException(dsValue.length, nR);
        }
        double[][] jacobianData = (double[][]) Array.newInstance(double.class, new int[]{nR, nC});
        for (int i3 = 0; i3 < nR; i3++) {
            int[] orders = new int[nC];
            for (int j = 0; j < nC; j++) {
                orders[j] = 1;
                jacobianData[i3][j] = dsValue[i3].getPartialDerivative(orders);
                orders[j] = 0;
            }
        }
    ...
    public void updateResidualsAndCost() {
        ...
        this.weightedResiduals = this.weightMatrixSqrt.operate((RealVector) new ArrayRealVector(res)).toArray();
    }

    /* access modifiers changed from: protected */
    public double computeCost(double[] residuals) {
        ArrayRealVector r = new ArrayRealVector(residuals);
        return FastMath.sqrt(r.dotProduct(getWeight().operate((RealVector) r)));
    }
    ...
    public void setUp() {
        ...
        this.f732jF = FunctionUtils.toMultivariateDifferentiableVectorFunction((DifferentiableMultivariateVectorFunction) getObjectiveFunction());
