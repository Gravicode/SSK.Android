...
public class RealMatrixFormat {
    ...
    public RealMatrix parse(String source, ParsePosition pos) {
        ...
        if (!CompositeFormat.parseFixedstring(str, trimmedPrefix, parsePosition)) {
            return null;
        }
        ...
        while (loop) {
            if (!rowComponents.isEmpty()) {
                CompositeFormat.parseAndIgnoreWhitespace(source, pos);
                if (!CompositeFormat.parseFixedstring(str, trimmedColumnSeparator, parsePosition)) {
                    if (trimmedRowSuffix.length() != 0 && !CompositeFormat.parseFixedstring(str, trimmedRowSuffix, parsePosition)) {
                        return null;
                    }
                    CompositeFormat.parseAndIgnoreWhitespace(source, pos);
                    if (CompositeFormat.parseFixedstring(str, trimmedRowSeparator, parsePosition)) {
                ...
                CompositeFormat.parseAndIgnoreWhitespace(source, pos);
                if (trimmedRowPrefix.length() != 0 && !CompositeFormat.parseFixedstring(str, trimmedRowPrefix, parsePosition)) {
                    return null;
                }
            }
            if (loop) {
                CompositeFormat.parseAndIgnoreWhitespace(source, pos);
                Number component = CompositeFormat.parseNumber(str, this.format, parsePosition);
                if (component != null) {
        ...
        CompositeFormat.parseAndIgnoreWhitespace(source, pos);
        if (!CompositeFormat.parseFixedstring(str, trimmedSuffix, parsePosition)) {
            return null;
        }
        if (matrix.isEmpty()) {
            parsePosition.setIndex(initialIndex);
