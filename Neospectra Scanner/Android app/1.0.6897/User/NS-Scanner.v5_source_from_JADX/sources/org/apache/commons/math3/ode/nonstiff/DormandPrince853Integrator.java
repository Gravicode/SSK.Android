...
public class DormandPrince853Integrator extends EmbeddedRungeKuttaIntegrator {
    ...
    public double estimateError(double[][] yDotK, double[] y0, double[] y1, double h) {
        ...
        double error1 = 0.0d;
        int j = 0;
        while (j < this.mainSetDimension) {
            double errSum1 = (yDotK[c][j] * E1_01) + (yDotK[5][j] * E1_06) + (yDotK[6][j] * E1_07) + (yDotK[7][j] * E1_08) + (yDotK[8][j] * E1_09) + (yDotK[9][j] * E1_10) + (yDotK[10][j] * E1_11) + (yDotK[11][j] * E1_12);
            double errSum2 = (yDotK[c][j] * E2_01) + (yDotK[5][j] * E2_06) + (yDotK[6][j] * E2_07) + (yDotK[7][j] * E2_08) + (yDotK[8][j] * E2_09) + (yDotK[9][j] * E2_10) + (yDotK[10][j] * E2_11) + (yDotK[11][j] * E2_12);
            double yScale = FastMath.max(FastMath.abs(y0[j]), FastMath.abs(y1[j]));
            if (this.vecAbsoluteTolerance == null) {
                error2 = error22;
                d = this.scalAbsoluteTolerance + (this.scalRelativeTolerance * yScale);
            } else {
                error2 = error22;
                d = (this.vecRelativeTolerance[j] * yScale) + this.vecAbsoluteTolerance[j];
            }
            double tol = d;
            double ratio1 = errSum1 / tol;
            error1 += ratio1 * ratio1;
            double ratio2 = errSum2 / tol;
            error22 = error2 + (ratio2 * ratio2);
            j++;
            c = 0;
        }
        double den = (error22 * 0.01d) + error1;
        if (den <= 0.0d) {
            den = 1.0d;
        }
