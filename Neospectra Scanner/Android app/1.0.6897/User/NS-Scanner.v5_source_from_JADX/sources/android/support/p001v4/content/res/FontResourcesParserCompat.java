...
import org.apache.poi.p009ss.util.CellUtil;
...
public class FontResourcesParserCompat {
    ...
    private static FamilyResourceEntry readFamily(XmlPullParser parser, Resources resources) throws XmlPullParserException, IOException {
        TypedArray array = resources.obtainAttributes(Xml.asAttributeSet(parser), C1088R.styleable.FontFamily);
        String authority = array.getString(C1088R.styleable.FontFamily_fontProviderAuthority);
        String providerPackage = array.getString(C1088R.styleable.FontFamily_fontProviderPackage);
        String query = array.getString(C1088R.styleable.FontFamily_fontProviderQuery);
        int certsId = array.getResourceId(C1088R.styleable.FontFamily_fontProviderCerts, 0);
        int strategy = array.getInteger(C1088R.styleable.FontFamily_fontProviderFetchStrategy, 1);
        int timeoutMs = array.getInteger(C1088R.styleable.FontFamily_fontProviderFetchTimeout, DEFAULT_TIMEOUT_MILLIS);
        array.recycle();
    ...
    private static FontFileResourceEntry readFont(XmlPullParser parser, Resources resources) throws XmlPullParserException, IOException {
        TypedArray array = resources.obtainAttributes(Xml.asAttributeSet(parser), C1088R.styleable.FontFamilyFont);
        int weight = array.getInt(array.hasValue(C1088R.styleable.FontFamilyFont_fontWeight) ? C1088R.styleable.FontFamilyFont_fontWeight : C1088R.styleable.FontFamilyFont_android_fontWeight, NORMAL_WEIGHT);
        boolean z = true;
        if (1 != array.getInt(array.hasValue(C1088R.styleable.FontFamilyFont_fontStyle) ? C1088R.styleable.FontFamilyFont_fontStyle : C1088R.styleable.FontFamilyFont_android_fontStyle, 0)) {
            z = false;
        }
        boolean isItalic = z;
        int resourceAttr = array.hasValue(C1088R.styleable.FontFamilyFont_font) ? C1088R.styleable.FontFamilyFont_font : C1088R.styleable.FontFamilyFont_android_font;
