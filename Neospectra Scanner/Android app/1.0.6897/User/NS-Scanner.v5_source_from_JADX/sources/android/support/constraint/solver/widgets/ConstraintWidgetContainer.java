...
public class ConstraintWidgetContainer extends WidgetContainer {
    ...
    public static ConstraintWidgetContainer createContainer(ConstraintWidgetContainer container, String name, ArrayList<ConstraintWidget> widgets, int padding) {
        ...
        if (padding > 0) {
            int maxPadding = Math.min(bounds.f31x, bounds.f32y);
            if (padding > maxPadding) {
                padding = maxPadding;
            }
            bounds.grow(padding, padding);
        }
        container.setOrigin(bounds.f31x, bounds.f32y);
        ...
        for (int i = 0; i < widgetsSize; i++) {
            ...
            if (widget.getParent() == parent) {
                ...
                widget.setX(widget.getX() - bounds.f31x);
                widget.setY(widget.getY() - bounds.f32y);
            }
        }
        return container;
    }
    ...
    /* JADX WARNING: Removed duplicated region for block: B:208:0x0548  */
    /* JADX WARNING: Removed duplicated region for block: B:209:0x054b  */
    /* Code decompiled incorrectly, please refer to instructions dump. */
    private void applyVerticalChain(android.support.constraint.solver.LinearSystem r43) {
        ...
        L_0x0446:
            ...
            goto L_0x0443
        L_0x045b:
            if (r33 != 0) goto L_0x0498
            if (r18 != 0) goto L_0x0498
        ...
        while (needsSolving2) {
            ...
            if (countSolve >= 8 || !this.flags[2]) {
                ...
                while (i3 < count) {
                    ...
                    maxX = Math.max(maxX, widget3.f26mX + widget3.getWidth());
                    maxY = Math.max(maxY, widget3.f27mY + widget3.getHeight());
                    i3++;
        ...
        if (widget.mHorizontalDimensionBehaviour != DimensionBehaviour.MATCH_CONSTRAINT || widget.mVerticalDimensionBehaviour == DimensionBehaviour.MATCH_CONSTRAINT || widget.mDimensionRatio <= 0.0f) {
            ...
            if (widget instanceof Guideline) {
                ...
                if (widget.mRight.mTarget != null && !rightWidget.isRoot()) {
                    ...
                    widget.mRightHasCentered = rightWidget.mRightHasCentered || !(rightWidget.mLeft.mTarget == null || rightWidget.mRight.mTarget == null || rightWidget.mHorizontalDimensionBehaviour == DimensionBehaviour.MATCH_CONSTRAINT);
        ...
        if (widget.mVerticalDimensionBehaviour != DimensionBehaviour.MATCH_CONSTRAINT || widget.mHorizontalDimensionBehaviour == DimensionBehaviour.MATCH_CONSTRAINT || widget.mDimensionRatio <= 0.0f) {
            ...
            if (widget instanceof Guideline) {
                ...
                if (widget.mTop.mTarget != null && !topWidget.isRoot()) {
                    ...
                    widget.mTopHasCentered = topWidget.mTopHasCentered || !(topWidget.mTop.mTarget == null || topWidget.mTop.mTarget.mOwner == widget || topWidget.mBottom.mTarget == null || topWidget.mBottom.mTarget.mOwner == widget || topWidget.mVerticalDimensionBehaviour == DimensionBehaviour.MATCH_CONSTRAINT);
    ...
    public void findWrapSize(ArrayList<ConstraintWidget> children, boolean[] flags2) {
        ...
        int j = 0;
        while (j < size) {
            ConstraintWidget widget = (ConstraintWidget) arrayList.get(j);
            if (!widget.isRoot()) {
        ...
        for (int j2 = 0; j2 < size; j2++) {
            ConstraintWidget child = (ConstraintWidget) arrayList.get(j2);
    ...
    private int countMatchConstraintsChainedWidgets(LinearSystem system, ConstraintWidget[] chainEnds, ConstraintWidget widget, int direction, boolean[] flags2) {
        ...
        if (direction == 0) {
            ...
            while (widget2.mRight.mTarget != null) {
                ...
                if (widget2.getVisibility() != 8 && widget2.mHorizontalDimensionBehaviour == DimensionBehaviour.MATCH_CONSTRAINT) {
        ...
        while (widget2.mBottom.mTarget != null) {
            ...
            i2 = 8;
            if (widget2.getVisibility() != 8 && widget2.mVerticalDimensionBehaviour == DimensionBehaviour.MATCH_CONSTRAINT) {
                if (widget2.mHorizontalDimensionBehaviour == DimensionBehaviour.MATCH_CONSTRAINT) {
                    flags2[0] = false;
                }
                if (widget2.mDimensionRatio <= 0.0f) {
                    flags2[0] = false;
                    if (count3 + 1 >= this.mMatchConstraintsChainedWidgets.length) {
                        this.mMatchConstraintsChainedWidgets = (ConstraintWidget[]) Arrays.copyOf(this.mMatchConstraintsChainedWidgets, this.mMatchConstraintsChainedWidgets.length * 2);
                    }
                    int count4 = count3 + 1;
                    this.mMatchConstraintsChainedWidgets[count3] = widget2;
                    count3 = count4;
                }
            }
